...
public struct ComposeSystem: AsyncParsableCommand {
    public static let configuration: CommandConfiguration = .init(
        ...
        subcommands: [ComposeSystemStatus.self]
    )
...
public struct ComposeSystemStatus: AsyncParsableCommand {
    ...
    public func run() async throws {
        let resolvedSocketPath = ServeDaemon.resolveSocketPath(override: socketPath)
        ...
        if daemonRunning {
            print("Daemon:    running (responded to /_ping in \(Self.milliseconds(for: elapsed))ms)")
